...
export default function Gallery() {
  ...
  useEffect(() => {
    // Fetch from server-side cached route — Supabase is called at most
    // once per 55 minutes (revalidate), not once per page visitor.
    fetch("/api/gallery")
      .then((r) => r.json())
  ...
  // Trigger GSAP after photos load
  useEffect(() => {
    if (loading) return;
  ...
  return (
    <section ref={sectionRef} className="bg-black py-24 md:py-40">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        {/* Empty state — only shows if Supabase returns nothing */}
        {!loading && photos.length === 0 && (
          <p
